...
| # | 标题 | 结构 | 情绪钩子 |
|---|------|------|---------|
| 1 | xxx | 冲突并置+情绪落点 | [读者心里会"咯噔"的点是什么] |
| 2 | yyy | 双画面对比 | [两个画面的情绪反差在哪] |
| 3 | zzz | 半句话+悬念 | [读者会追问"然后呢？"的点] |
| 4 | aaa | 身份命中+状态 | [哪类读者会觉得"这说的是我"] |
| 5 | bbb | 口语碎片 | [像从嘴里蹦出来的哪句话] |

标题规则：

**核心原则：标题里要有"人"，不能只有"事"。结构反差让人好奇，情绪钩子让人心动——两者缺一不可。**

**长度：8-18 字。** 太短（<8）信息流里等于没说话；太长（>18）读起来像摘要。

**5 种结构（按传播力排序）：**

| # | 结构 | 形式 | 好例子 | 差例子（只有事没有人） |
|---|------|------|--------|-------------------|
| 1 | 冲突并置+情绪落点 | A 和 B 矛盾，第三句拐进人的状态 | "顶风出门，海边没风，心里有风" | "大风天的海边野餐" |
| 2 | 双画面对比 | 两个画面并置，一轻一重（或一反一正），情绪落差即钩子 | "不想出门的早上，不想回家的下午" | "取消行程后的意外收获" |
| 3 | 递进短句 | 2-3 个短句递进，最后一句突然从外部场景拐进人的内心 | "风停了，潮涨了，人睡不着" | "退潮、礁石和凌晨三点" |
| 4 | 身份命中+状态 | [身份标签] + [只有这群人才懂的状态] | "孩子在礁石上闯关，我在床上翻来覆去" | "带娃中年人的周末" |
| 5 | 口语碎片 | 像从对话/内心独白里截出来的半句，带情绪温度 | "又梦到老家了" | "清明思乡" |

**情绪钩子自检（生成后逐条过）：**
1. 标题里有没有"人的状态"？纯事件/纯场景不通过——必须能感受到一个人的情绪
2. 读者划过这个标题，心里会不会"咯噔一下"或"嘴角一动"？没有体感反应就是缺钩子
3. 两个人转发到朋友圈，是否因为不同的理由？好标题允许多重解读
4. 标题承诺的情绪，文章里是否真的有？（诚实测试）

**结构选择决策：**
- 素材有前后反转/出乎意料 → 优先「冲突并置」或「双画面对比」
- 素材有渐进的情绪变化 → 「递进短句」
- 素材与特定人群强相关 → 「身份命中」
- 素材情绪浓但事件简单 → 「口语碎片」
...
- 不用营销号套路（"震惊""99%的人不知道"）
- 不用抽象大词（"认知升级""底层逻辑"）
- 不写成完整叙事句（"我取消了行程反而遇到了最好的一天"——这是摘要不是标题）
- 不写成单个抽象词（"退潮""礁石"——信息量为零）
- 标题必须与内容一致——"用夸张的方式说真话"
...
操作步骤：
1. **先 pull rebase 同步远端，再读取计数器**（避免多机/多设备发文时本地计数滞后导致 WeChat 发错号）：
   ```bash
   ...
   然后读取计数器文件，获取当前值 N
   - rebase 冲突 → 停下询问 jdy
   - 网络失败 → 告知 jdy 是否继续；继续则用本地值（但 step 8 push 可能被 reject）
   - `--autostash` 兜底 Phase 1.2 可能留下的 `writing-style-profile.md` 脏改动
   - **如果文件不存在或为空，必须询问用户当前序号是多少，禁止从 vault 文件数量推断**（vault 不包含全部历史文章）
2. 发布标题 = `{N+1}/1000 {原标题}`
3. 创建发布版本文件（见 6.3.1）
4. 追加固定引流段落到发布版本（见 6.3.2）
5. 调用 `baoyu-post-to-wechat` skill 发布，必须带以下参数：
   - `--title`：带序号的标题
   - `--summary`：文章前 1-2 句话（**不含标题**，避免摘要重复）
   - `--cover`：封面图路径
   - `--author`：作者名
6. **API 失败 fallback**：如果 API 方式失败（如 IP 白名单、access token 错误），自动提示用户切换浏览器方式重试
7. 发布成功后，将计数器更新为 N+1
8. **commit + push 计数器到远端**（pull rebase 已在 step 1 完成，此处不再重复）：
   ```bash
   ...
   - push 被拒（远端在本次发文期间又被另一台机器更新）→ 停下询问 jdy，**禁止** `--force`；此时 WeChat 已经发布，本地号已对，需手动 rebase + 解决冲突再 push
   - 网络失败 → 告知 jdy 不阻塞 vault 流程；下次发文 step 1 pull 时会带出累积的 commit
9. 删除 `-publish.md` 临时文件，避免污染 vault

#### 6.3.1 发布版本文件管理

发布版本是原始 md 文件的副本，仅用于追加引流内容，发布后删除。

- 文件命名：`{原文件名}-publish.md`（如 `20260324 吃不到的胡萝卜-publish.md`）
- 内容 = 原始 md 文件全文（含配图），**去掉末尾的发布链接行**（`> 发布链接：...`）

#### 6.3.2 末尾引流（仅发布版本，不写入保存的 md 文件）

在发布版本末尾追加微信二维码图片 + 固定引流段落。
...
做这个表不是因为我有什么 alpha，是反撸太多次了——花了时间精力最后倒贴，逼出来的工具。我用它劝自己：这个项目不值得跑。
...
**规则**：
- **逐字使用，不改写、不二次创作**——这是 jdy 确认过的固定品牌段落
- 二维码图片用 inline HTML 控制居中和宽度（markdown 无法控制图片大小）
- 文案用纯 markdown 段落，让公众号渲染引擎使用正常正文样式（不要居中，不要灰字小字——这段是 jdy 真诚的话，不是免责声明）
- 不需要询问用户确认文案内容，直接追加

**二维码图片绝对路径**：`60_Output/公众号/ken_wang.jpg`
**相对路径计算**：根据发布文件所在目录自动计算。发布文件在 `202603/` 下时为 `../ken_wang.jpg`，在 `公众号/` 根目录下时为 `./ken_wang.jpg`
...
**如果用户暂时没有 URL**（比如还没发布），回复"稍后补充"即可跳过，但必须明确询问，不能默认跳过。
...
- 内容 = 公众号原文（含配图），但**排除封面图**（封面通过 `--cover` 单独传）和**排除发布链接行**
