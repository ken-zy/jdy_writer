...
- **封面必须上传 R2**，key 命名 `illustrations/<YYYYMMDD>-cover.png`（多版本时用 `-cover-v2.png`）：
  ```bash
  npx wrangler r2 object put "obsidian-images/illustrations/<YYYYMMDD>-cover.png" \
    --file="60_Output/公众号/<YYYYMM>/attachments/cover.png" \
    --content-type="image/png" --remote
  ```
  公网 URL：`https://pub-48c71f8223274ef1bc0c403d79a30f15.r2.dev/illustrations/<YYYYMMDD>-cover.png`
- **封面 R2 URL 必须插入正文开头**——紧跟 `# 标题` 后，空一行，作为正文第一项内容：
  ```markdown
  # 文章标题

  ![](https://pub-48c71f8223274ef1bc0c403d79a30f15.r2.dev/illustrations/<YYYYMMDD>-cover.png)

  正文第一段……
  ```
- 封面图同时作为 Phase 6.3 发布参数 `--cover` 使用（传本地 `attachments/cover.png` 路径或 R2 URL 均可）
- Phase 7.1 生成 X Article 时，从公众号 md 复制内容时**自动剥离开头的封面图行**（X 通过 `--cover` 单独传，避免重复）
...
- 段落开头的 `---` 是与正文的视觉分隔线，公众号会渲染成一条细横线，避免读者把引流块误读为正文延续
- 二维码图片用 inline HTML 控制居中和宽度（markdown 无法控制图片大小）
- 文案用纯 markdown 段落，让公众号渲染引擎使用正常正文样式（不要居中，不要灰字小字——这段是 jdy 真诚的话，不是免责声明）
- 不需要询问用户确认文案内容，直接追加

**二维码图片源文件**：`/Users/jdy/Documents/skills/jdy_writer/ken_wang.jpg`

**发布前处理**：
- 若 vault 中 `60_Output/公众号/ken_wang.jpg` 不存在，先从 skill 源文件复制过去
- 若已存在，直接复用；不要从外部临时目录、Downloads 或对话附件重新找图
- 发布版本中的相对路径根据发布文件所在目录自动计算。发布文件在 `202603/` 下时为 `../ken_wang.jpg`，在 `公众号/` 根目录下时为 `./ken_wang.jpg`
